...
// #44 / R0: regressions assert the correct product result, not the known defect.
// Every failure stays RED. Direct fixture entry is labelled and never counted as
// incoming-edge or shared-state proof. No production/sponsor mutations are used.
...
async function nextAction(page, name) {
  ...
  for (let i = 0; i < await choices.count(); i += 1) {
    if (await choices.nth(i).isVisible()) {
...
async function stillRecovered(page) {
  ...
  for (let i = 0; i < await usable.count(); i += 1) {
    assert.equal(await usable.nth(i).isVisible(), false, "Recovered Friday Yoga became usable again");
  }
}
...
const cases = [
  ...
  { id: "NAV-05-authorization-back", requirement: "F-08", entry: "clicked holder journey", prepare: mayaWaiting,
    probe: async (p) => {
      ...
      assert.equal(new URL(p.url()).pathname, "/product-preview", "Back ejected the pending authorization to another product/page");
      await p.goForward({ waitUntil: "networkidle" });
      await visible(p, "Waiting for your Ledger.");
    } },
  { id: "NAV-06-payment-history", requirement: "F-02/F-08", entry: "clicked acquirer journey", prepare: bobPending,
    probe: async (p) => {
      await p.goBack({ waitUntil: "networkidle" });
      assert.equal(new URL(p.url()).pathname, "/product-preview", "Back ejected the pending payment");
      await p.goForward({ waitUntil: "networkidle" });
      await visible(p, "Confirming your 45 USDC commitment.");
    } },
  { id: "NAV-07-checkin-header", requirement: "F-03", entry: "direct fixture: check-in-ready; then actual Check in", prepare: async (p) => {
      await entry(p, "xc2-bob-ready"); await action(p, "Check in"); await visible(p, "You’re checked in.");
  ...
  { id: "NAV-08-fulfilled-header", requirement: "F-03", entry: "direct fulfilled fixture; readback only, NOT fulfil-action proof", prepare: async (p) => {
      await entry(p, "xc2-provider-fulfilled"); await visible(p, "Friday Yoga was fulfilled for Bob.");
    }, probe: async (p) => { await action(p, "Today", "link", p.locator("header")); await visible(p, /[Ff]ulfilled/); } },
  ...[
    ["maya", "bookings", "Maya Keller"], ["bob", "xc-bob-success", "Bob"], ["provider", "xc-provider-policy", "Studio A"],
  ].map(([actor, view, identity]) => ({ id: `IDENTITY-${actor}`, requirement: "F-05", entry: `direct fixture: ${view}; visible identity only`,
    prepare: (p) => entry(p, view), probe: (p) => visible(p, identity, p.locator("header")) })),
  { id: "BRIDGE-01-use-checkin", requirement: "B1/F-06", entry: "clicked acquirer journey", prepare: bobSuccess,
    probe: async (p) => {
      await action(p, "Use booking");
      await visible(p, /Check in for Friday Yoga|Friday Yoga is ready for later|You[’']re checked in/i, p.locator("main h1"));
    } },
  { id: "BRIDGE-02-recovery-receipt", requirement: "B2/F-07", entry: "clicked holder journey", prepare: mayaSuccess,
    probe: async (p) => {
      await nextAction(p, /view (activity|receipt)|recovery receipt|view recovery/i);
      await visible(p, "You recovered 45 USDC"); await visible(p, /activity|receipt|history/i);
    } },
  { id: "BRIDGE-03-provider-handoff", requirement: "B3", entry: "direct provider-final fixture; continuation only", prepare: async (p) => {
      await entry(p, "xc-provider-final"); await visible(p, "Bob is now the current holder.");
    }, probe: async (p) => {
      await nextAction(p, /today|fulfil|manage booking|open booking/i);
      await visible(p, "Bob"); await visible(p, /attendance|check.in|fulfil/i);
    } },
  { id: "BRIDGE-04-provider-action", requirement: "B4/F-04", entry: "direct expected-guest fixture; missing-action probe only", prepare: async (p) => {
      await entry(p, "xc2-provider-pending"); await visible(p, "Bob is the expected guest");
    }, probe: async (p) => {
      await nextAction(p, /record attendance|mark.*fulfilled|complete.*session|fulfil booking|confirm attendance|check in bob/i);
      await visible(p, /recording|pending|checked in|fulfilled|could not|did not/i);
    } },
];
...
const sourceDigests = {};
for (const file of ["app/product-preview/GoldenRecoveryClient.tsx", "app/product-preview/Xc01Client.tsx", "app/product-preview/CompletionClient.tsx", "app/product-preview/ProviderLifecycleClient.tsx", "components/SiteHeader.tsx"]) {
...
try {
  for (const viewport of viewports) {
    for (const test of cases) {
      ...
      // Only loopback reads and in-browser fixture actions; never call a sponsor.
      ...
      const capture = async (suffix) => {
        const filename = `${prefix}-${suffix}.png`;
        await page.screenshot({ path: path.join(out, filename), fullPage: true });
        record.screenshots.push(filename);
      };
      try {
        await test.prepare(page);
        await capture("before");
        ready = true;
        await test.probe(page);
        record.status = record.consoleErrors.length ? "RUNTIME_ERROR" : "PASS";
      } catch (error) {
        record.status = ready ? "FAIL" : "SETUP_ERROR";
        record.error = scrub(error.message);
      } finally {
        ...
        await capture("after").catch((e) => { record.evidenceError = scrub(e.message); record.status = "EVIDENCE_ERROR"; });
        results.push(record);
        console.log(`${record.status} ${prefix} [${record.requirement}]`);
        await context.close();
      }
    }
  }
...
const report = {
  ...
  scope: "R0 navigation/bridge regression baseline; not a complete R1-R4 acceptance suite",
  wholeProductClearance: false, sourceDigests, counts, results,
  remainingEvidence: ["R1 persistent shared state and interrupted-action semantics", "R2 complete bridges including fulfilment pending/error/result", "R3 editable provider/customer entry scope", "R4 single-seed cross-actor scenario and duplicate/race tests", "R5 independent exact-head PNG and interaction review; human approval"],
};
await writeFile(path.join(out, "results.json"), JSON.stringify(report, null, 2));
const summary = `# #44 UX integrity evidence\n\nExact head: \`${sha}\`\n\n${report.scope}. A passing R0 suite alone never establishes whole-product completion.\n\n| Case | Viewport | Result | Requirement |\n| --- | --- | --- | --- |\n${results.map((r) => `| ${r.id} | ${r.viewport} | ${r.status} | ${r.requirement} |`).join("\n")}\n\nSee results.json for explicitly labelled fixture entries and paired screenshots. Setup/runtime/evidence errors are not product-defect reproductions.\n`;
