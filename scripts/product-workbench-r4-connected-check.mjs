...
try{for(const [name,viewport] of [['desktop',{width:1440,height:1000}],['mobile',{width:390,height:844}]]){
 ...
 try{
  ...
  await visible(maya,'32 USDC was not accepted.');assert.equal((await storedFixture(maya)).activeMinimum,40);await snap(maya,'32-denied');
  ...
  await location(staleBob,'xc-payment-pending');assert.equal((await storedFixture(bob)).continuation.paymentAttempts,1);await history(bob);await snap(bob,'payment-interrupted-resumed');
  // Both tabs refresh the same pending operation; no second commitment.
  ...
  await location(staleBob,'xc-bob-success');await unchanged(staleBob,acquired);await history(bob);await snap(bob,'bob-acquired');
  ...
  await entry(studio,'xc2-provider-pending');await duplicate(studio,'Fulfil booking');await location(studio,'xc2-provider-fulfilment-pending');await environment(studio,{fulfilmentResponse:'unknown'},record);
  ...
  await click(studio,'Retry fulfilment');await location(studio,'xc2-provider-fulfilment-pending');await click(studio,'Refresh fulfilment');await location(studio,'xc2-provider-fulfilled');await history(studio);await snap(studio,'service-fulfilled-once');
