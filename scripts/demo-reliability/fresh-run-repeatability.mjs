...
import { runRecoveryCycle, freshSigner, seedAppState, assertGuardedLoaderRejectsMissingContext } from "./recovery-flow.mjs";
...
console.log("\nguard integrity — the loader must reject missing context");
const guard = await assertGuardedLoaderRejectsMissingContext({
  redis, mandateId: RUN2.mandateId, ownerId: "maya-keller",
});
let guardFailures = 0;
for (const g of guard) {
  console.log(`  ${g.rejected ? "PASS" : "FAIL"}  ${g.label}: ${g.detail}`);
  if (!g.rejected) guardFailures += 1;
}
if (guardFailures > 0) {
  console.log("\nFRESH-RUN REPEATABILITY: FAIL — guarded loader accepted missing context");
  process.exit(1);
}
