...
export default function ProviderR3Client() {
  ...
  if (view === "xc3-provider-session-published") {
    ...
    return <Frame title="Friday Yoga is open for bookings." intro="These are the saved provider/session rules currently published to the fixture runtime. Draft edits cannot change them until Studio A saves again.">
  ...
  return <Frame title="Edit Friday Yoga before publishing." intro="Draft changes persist for Studio A while you move around the preview, but customer/runtime rules stay on the last saved version until Save and publish succeeds.">
    <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
      <Panel>
        ...
        <div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={publish} className={primaryButton}>Save and publish</button><button type="button" onClick={() => router.push("/product-preview?view=xc3-provider-inventory")} className={secondaryButton}>Back to inventory</button></div>
      </Panel>
      <Panel><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Published/runtime truth</p><h2 className="mt-2 text-xl font-semibold text-slate-950">What customers are governed by now</h2><div className="mt-5 grid gap-3"><Fact label="Session" value={`${state.published.sessionTitle} · ${state.published.startTime}`} /><Fact label="Capacity" value={`${state.published.capacity} places`} /><Fact label="Transfer cutoff" value={`Friday · ${state.published.transferCutoff}`} /><Fact label="Eligibility" value={state.published.eligibilityRule} /><Fact label="Recovery" value="Allowed under published provider policy" /></div><p className="mt-5 text-sm leading-6 text-slate-600">Invalid input or a failed save leaves every published value above unchanged. No individual recovery asks Studio A for approval.</p></Panel>
    </div>
  </Frame>;
}
