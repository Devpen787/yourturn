...
export default function ProviderR3Client() {
  ...
  if (view === "xc3-provider-session-published") {
    ...
    return <Frame title="Friday Yoga is open for bookings." intro="Published settings govern this prepared Friday Yoga fixture. Maya already holds the booking; this preview does not demonstrate an initial purchase.">
      <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
        <Panel><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-slate-950">{p.sessionTitle} · {p.startTime}</p><p className="mt-1 text-sm text-slate-600">Studio A · Zürich</p></div><span className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-800 ring-1 ring-emerald-100">Published</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2"><Fact label="Capacity" value={`${p.capacity} places`} /><Fact label="Transfer cutoff" value={`Friday · ${p.transferCutoff}`} /><Fact label="Eligibility" value={p.eligibilityRule} /><Fact label="Recovery" value={p.eligibilityRule === "Recovery paused" ? "Paused by Studio A" : "Allowed under published provider policy"} /></div></Panel>
      ...
      <div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={() => router.push("/product-preview?view=xc3-provider-session-edit")} className={secondaryButton}>Edit session & rules</button><button type="button" onClick={() => router.push("/product-preview?view=xc3-provider-today")} className={primaryButton}>Open booking activity</button></div>
    </Frame>;
  }
