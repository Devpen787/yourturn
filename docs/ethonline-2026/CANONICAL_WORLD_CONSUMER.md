...
Initial a6bb2f74 received REVISE (#16 5648171468). The reviewer reproduced (1) a factory endpoint different from the registry's signed resource and (2) a delayed AgentBook lookup making an old issue time appear fresh through the verifier's later verification timestamp. The successor requires a fixed trusted `dependencies.resourceUri`; the HTTP factory requires it equal the actual configured URI and snapshots it, while new and status paths compare the current binding before nonce consumption. Consumer-local checks retain the original signed issuedAt/expiry across every subsequent World gate and reject verification taking five seconds or more. The independently qualified verifier is unchanged.

The initial hosted focused run34714018435 failed its baseline Agent Kit check because synthetic public account IDs were omitted; previous model/Redis/build/tsc steps passed but artifact upload did not run. The successor supplies the same explicit fixture IDs0.0.9001/0.0.9002 used by the cleared shared foundation workflow. No secret or real account configuration is loaded. Four added regression cases preserve both source findings, including the existing-operation status path. Fresh CI and separate re-review are required.
