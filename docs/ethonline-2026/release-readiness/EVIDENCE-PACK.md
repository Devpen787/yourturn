...
The current head pins the physical qualification contract at `docs/ethonline-2026/ledger/DEVICE_QUALIFICATION_RUNBOOK.md`: one identical server-prepared Recovery Mandate is intended to pass through on-device reject → host cancel → on-device approve, followed by reviewer-safe bundle checks and downstream wrong-signature/single-activation/replay checks. The contract/tooling is **CI GREEN**, but the real ceremony is **CURRENTLY PAUSED BY A SEPARATE DEPENDENCY-SECURITY HOLD**. Local public-address verification succeeded and was redacted; it is not a Recovery Mandate signature, activation, or `LIVE/DEVICE` proof. Security #16 comment `5634713735` records the precautionary hold after the isolated helper audit surfaced 7 entries (6 moderate, 1 high) and the root install surfaced 32 entries including 2 critical whose exact reachability/disposition is still pending. This does not reopen SEC-LEDGER-005/006 and does not establish compromise. Do not load ceremony credentials, start the ceremony app, sign/activate a mandate, or present the runbook as currently human-ready until the exact runtime dependency graph is independently Security-cleared. Physical identical-mandate DMK approve plus reject/cancel remains RED until that clearance and a successful real session.
...
Current H2/new NFT+USDC recovery remains `CI/LOCAL + Security`; canonical 45-USDC `LIVE/TESTNET` evidence is RED. The last verified public preflight recorded `0.0.8504405` at 19.98 testnet USDC, which implied a historical 25.02 shortfall, but a later top-up has been reported. Do **not** repeat 25.02 as the current shortfall or claim sufficient liquidity until a fresh public balance/role check is captured. After prerequisites are verified, the existing canonical 45-USDC workflow and independent review of that exact artifact remain required. H0/H1 retain their previously accepted LIVE/TESTNET evidence. Do not call the combined customer recovery `atomic` until one actual independently reviewed Hedera transaction proves both movements.
...
- Hedera H2/new NFT+USDC recovery: `CI/LOCAL + Security`; canonical 45-USDC `LIVE/TESTNET` artifact remains RED. Reported top-up is not yet a fresh verified balance, and the historical 25.02 shortfall is not current truth until rechecked.
...
- **Evidence pack: RED.** The contract exists, but Hedera 45-USDC LIVE evidence, World signed-route/canonical-mandate bridge, Ledger dependency-security clearance plus LIVE/DEVICE evidence, sponsor-backed fixture replacement and the final integrated adversarial E2E are still missing.
