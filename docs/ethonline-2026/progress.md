...
### 2026-09-12 — exact indexed Hedera settlement receipt reader

- Branch/base: `feature/ethonline-hedera-receipt-reader-qual` from independently cleared canonical resolver `20323ca7a6d0fcb3ccfb2ee86e5413fdaedff229`; claim #16 5648247742.
- Change: fixed-origin, GET-only transaction-by-ID reader verifies the exact v2 transaction body, memo, delegated NFT movement, complete buyer-gross/seller-net/collector USDC ledger, bounded fee-only HBAR ledger and two stable indexed reads. JSON numeric tokens remain lexical strings before validation, so int64 financial values never pass through IEEE754. Receipt reconciliation grants no authority and remains valid after proposal expiry.
- Verification actually run locally before the first commit: 84 focused synthetic Mirror response cases and standalone TypeScript PASS. Independent review then reproduced duplicate-object-key last-value ambiguity in the response parser, so the successor rejects decoded duplicate keys at every nesting level and adds root/transaction/transfer, escaped-equivalent and identical-key regressions plus a valid escaped-string check. The matrix covers role/economics/body/fee/precision/completeness/drift/transport/time failures. No live Mirror request was executed. Exact successor hosted evidence and independent retest are required.
- Evidence/limits: `HEDERA_INDEXED_RECEIPT_READER.md` and the hosted JSON artifact. Synthetic public fixtures only; no credential, key, device, signature, transaction, funds or current account selection. Unsupported batch, high-volume, pagination, staking rewards and ambiguous fee movements fail closed.
- Claim impact: receipt-observation candidate only. It does not prove current authority, consensus-synchronous finality, a submitted transaction, or sponsor-LIVE qualification.
- Exact next action: publish the bounded branch, inspect exact CI and artifact, then obtain separate independent review before any integration consumption.
