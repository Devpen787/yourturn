...
### 2026-09-12 — published provider policy with exact-scope ownership and CAS
- Branch/base: `feature/ethonline-foundation-provider-policy-qual` from independently cleared `4b4189a02c17bcb4fe37991914694742db0f7620`; assigned claim #16 `5647773575`.
- Change: isolated server-only provider/token/serial published policy store with authenticated issuer input and mandatory trusted enrollment resolver. Strict policy records bind enrollment version/fingerprint, preserve explicit ALLOW/BLOCK/REVIEW, net floor/null and owner-selected 0–100% royalty fractions. Atomic Redis Lua compares full predecessor, increments durable version and checks Redis TIME before SET; expired records retain versions. No fallback to Product/localStorage, legacy plan, booked snapshot or a default owner. No routes or legacy core edits.
- Verification actually run: 45 local MODEL cases PASS, production build PASS, post-build TypeScript PASS, baseline Agent Kit check PASS using canonical CI public synthetic account IDs. Existing locked Ledger dependency graph reused via node_modules symlink; fresh install/real Redis remain hosted checks pending exact publication. Local model EVAL simulation is explicitly not actual Redis. Dedicated workflow runs disposable Redis 7.4.2 and uploads exact-SHA assertions/digest.
- Notable boundaries: issuer authentication and provider enrollment are trusted caller dependencies, not implemented by this store. External enrollment and Redis cannot be atomically locked by a callback; point-in-time checks and indeterminate post-commit errors are explicit. Changed enrollment makes the old fingerprint unusable. Publish timeout may already have committed: reload/reconcile, never restore old facts or blindly retry. No HTTP, signing, chain transaction, hardware, real credential or production Redis write.
- Contract/evidence: `docs/ethonline-2026/PUBLISHED_PROVIDER_POLICY_CONTRACT.md`; local model evidence is not sponsor qualification. Scope allows policy validity up to 366 days and future explicit republication; ownership migration and final consumer effect-boundary checks remain outside this increment.
- Exact next action: push qualification SHA; inspect hosted real Redis, install/build and Continuity; independent source/security review before any consumer adopts the primitive. No acceptance flag self-promoted.

### 2026-09-12 — immutable public enrollment registry
- Branch/base: `feature/ethonline-foundation-public-enrollment-qual` from independently cleared provider store `3f1c2de645ecab51a0d38f7695360a4b2437d0a2`; claim #16 `5647885248`.
- Change: one standalone module snapshots a trusted server public manifest with exact provider/token/serial ownership and owner/internal-agent World/Hedera public bindings. Strict schema/version/scope/time validation, duplicate/conflicting booking rejection, collision-safe tuple keys, explicit expiry/revocation, monotonic observed clock, frozen inputs/results and no fallback. Provider lookup returns exactly the existing cleared store's enrollment shape; agent lookup returns registered public facts/provenance only. No pending canonical projection import, real records, default identities, routes, provisioning/auth framework or existing core edits.
- Verification actually run: 67 synthetic registry/model-storage checks PASS, production build PASS, post-build TypeScript PASS, baseline Agent Kit check PASS with existing public synthetic CI IDs. Tests consume the actual cleared provider publish/load module through the registry callback and show enrollment expiry denying a still-persisted policy. Existing provider dependency graph reused through symlink; fresh locked install/exact hosted CI pending push. No real Redis execution or production provisioning claimed by this matrix.
- Boundaries: immutable instances do not observe later configuration changes. Trusted owner-reviewed provisioning must replace prior instances for revocation, prevent deployment rollback and version changed bindings. Local clock monotonicity is not durable deployment fencing. Registration is not execution authority; actual identity selection, current resolver, effect coordination and canonical mapping review remain separate.
- Contract: `docs/ethonline-2026/PUBLIC_ENROLLMENT_REGISTRY_CONTRACT.md`. No credentials, hardware, signatures, chain/state writes, dependencies or Product/Goldens changed; no acceptance self-promotion.
- Exact next action: publish this qualification ref, inspect exact CI and hand to independent review before consumer adoption or real manifest provisioning.
