...
## 2026-09-12 — operation-owned Hedera preparation and durable retention

- Branch/base: `feature/ethonline-hedera-prepare-retain-qual` from independently cleared retained-output source `4f7bb4930139b92fa2ee1e223b76c406af1bad79`; claim #16 5648247742. Exact Hedera inputs are copied from cleared resolver `20323ca7` and payment-record `dddb0a52`; the patched Ledger package and lockfile remain byte-identical to the base.
- Change: server-only adapter revalidates the claimed operation-owned Ledger mandate and the complete canonical Hedera intent, validates Bob's native signature, atomically wins the sole begin-effect transition, calls the actual `RETURN_BYTES` preparation helper once, durably retains exact unsigned bytes, then attaches immutable output references. Started-without-output and unknown outcomes remain locked; receipt-only reads never reprepare.
- Verification actually run locally before commit: exact-source/graph check PASS with 31 pinned imports; 23 focused model cases PASS using the real SDK helper; production build and standalone TypeScript PASS. The focused matrix covers concurrency, authority/payment/eligibility drift, begin/reservation/retention uncertainty, revocation, expiry and receipt-only recovery. Actual Redis execution is hosted-only because no local Redis service is available.
- Evidence/limits: `HEDERA_PREPARE_RETAIN_ADAPTER.md` and exact hosted logs. Synthetic public keys and fixture records only. No World verification, credential, device, signature ceremony, transaction submission, receipt or funds. The current World consumer owns a separate begin transition and requires a later reviewed single-begin interface successor before integration.
- Claim impact: bounded preparation/retention candidate only; it returns unsigned retained bytes with `executionPermit: false` and cannot establish sponsor-LIVE qualification.
- Exact next action: publish exact source, inspect hosted disposable-Redis and Continuity results, obtain independent review, then compose a single-begin World successor without changing either cleared core.

## 2026-09-12 — trusted current-payment candidate validation

- Branch/base: `feature/ethonline-hedera-payment-validator-qual` from independently cleared prepare-and-retain source `9e8dc85888b5efacb16872b80b74ea0ed9eba88a`; claim #16 `5648433409`.
- Mission/change: add the mandatory trusted callback for `publishCurrentPaymentRecord`. It binds a server-selected operation and authenticated owner to the guarded stable Ledger projection, binds the publisher to the provider enrollment issuer, checks the owner-reviewed public registry/current provider policy/current business eligibility, requires two matching complete indexed-chain reads, and validates exact v2 quote/roles/gross/royalty/net/native-payer/transaction/deadline facts. It returns only a stable authority fingerprint and the earliest authority deadline.
- Verification actually run locally before commit: 49 focused model cases PASS; production TypeScript PASS. The source-integrity case pins package/lock and all consumed cleared cores byte-for-byte. Hosted workflow adds the same matrix with actual disposable Redis guarded sources, inherited payment/projection/resolver/registry/policy/preparation/signing/royalty/Ledger Redis regressions, locked install, build, TypeScript and baseline Agent Kit check.
- Evidence/limits: `HEDERA_PAYMENT_CANDIDATE_VALIDATOR.md` and exact hosted logs after publication. Synthetic public keys and mocked public Mirror responses only. The validator does not write payment state, claim/begin an operation, prepare, verify World, sign, submit, read a receipt or move funds. Business eligibility remains a mandatory server-owned callback. Royalty has no hard-coded ten-percent rule. The inherited whole-second `.000000000` transaction-ID contract is preserved; nonzero nanos remain denied.
- Claim impact: bounded candidate-validation source only; no execution permission or sponsor-LIVE qualification. The builder has not independently reviewed it.
- Exact next action: publish the exact source, inspect the hosted disposable-Redis and Continuity results, and obtain separate independent review before supplying this callback to a production payment publisher.
