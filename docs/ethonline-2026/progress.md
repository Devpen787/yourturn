...
### 2026-09-12 — durable current quote/payment record
- Branch/base: `feature/ethonline-hedera-payment-record-qual` from independently cleared resolver `20323ca7a6d0fcb3ccfb2ee86e5413fdaedff229`; claim #16 5648175305.
- Change: dedicated immutable operation/owner/provider/quote/v2 commitment record with authenticated server publisher + mandatory current candidate validator; permanent NX publish, owner/version CAS revoke via Redis TIME, separate current and reconciliation reads. No TTL/resurrection or default validator/eligibility. Reviewed source and dependencies unchanged.
- Verification actually run locally: 49 focused model checks, 59 inherited canonical resolver checks, production build, standalone TypeScript and baseline Agent Kit PASS with unchanged existing locked dependencies. Fresh hosted locked install and real Redis remain pending at commit; no local Redis executable is installed. Dedicated CI covers actual Lua plus inherited policy/payment/signing boundaries.
- Limits: persistent record primitive only. Mandatory publisher/candidate validation and authoritative eligibility/provisioning remain concrete integration dependencies. Cross-store validation is point-in-time; dispatched-write uncertainty is explicit and cannot authorize another effect. No credentials/device/signing/funds/production write.
- Next: exact hosted CI and independent Security review before adoption; no sponsor/acceptance self-promotion. See `HEDERA_CURRENT_PAYMENT_RECORD.md`.
