...
### 2026-09-12 — bounded public Hedera indexed-state reader
- Branch/base: `feature/ethonline-hedera-chain-state-qual` from independently cleared `e4e67d91aa4abdcaffd2ca55002546cbae4638dc`; assigned claim #16 `5647686417`.
- Mission/change: new isolated public Testnet Mirror GET adapter with explicit caller-selected roles/token/serial/funding amount. Two stable observation passes read exact serial owner/spender, absence of broad NFT allowance, native funding public key, account status, booking/USDC metadata, holder/receiver token relationships and any royalty collector USDC relationship. Owner-selectable single royalty metadata is preserved; provider policy is not inferred from it. Unsupported fee schedules/keys, incomplete exact queries, changed facts, unsafe numbers, unusable associations, body/timeout/read-window violations and stale index watermarks deny.
- Source/schema evidence: official `https://testnet.mirrornode.hedera.com/api/v1/docs/openapi.yml` plus public GET response shapes for USDC metadata, BOOKED metadata/NFT, account native key, token relationship and exact NFT-allowance filtering. Public inspection was read-only; no successful real-role adapter run is claimed.
- Verification actually run: 58 focused mock GET cases PASS; production build PASS; post-build TypeScript PASS; 87 inherited exact-payment semantic cases PASS; baseline `hedera:agent-check` PASS with the same public synthetic account IDs as Continuity CI. Initial empty-environment baseline agent check failed because its configured allowance fixture IDs were absent; corrected command supplied fixture IDs, without changing that checker. Local dependency graph reused via symlink to the existing locked install; fresh install and exact CI are pending publication. Initial TypeScript target/type diagnostics were corrected before the final successful run.
- Evidence/limits: MOCK_PUBLIC_GETS_ONLY, not settlement, signing or final canonical resolver. Latest indexed transaction age and fetch/read times are distinct, explicitly not proof that Mirror is synchronous with consensus. Two matching observations are not a chain lock; executor/registry/provider/mandate and external signer boundaries remain separate. No credentials, env files, hardware, real signatures, transactions, deployment or integration were added or used. No acceptance booleans promoted.
- Exact next action: push this qualification candidate, inspect exact Continuity and focused workflow results, then independent source/security review before any consumer imports this reader.
