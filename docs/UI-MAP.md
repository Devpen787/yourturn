...
| `/resale/[serial]` | `app/resale/[serial]/page.tsx` | Customer recovery + resale handoff page | `ResaleClient`, `RecoveryConciergePanel`, `WalletBudgetConnector`, `RecoveryProofCard` | Signed-in guest previews and approves Concierge recovery listing or release/refund, can optionally connect a Hedera wallet to approve a bounded USDC allowance budget, sees refreshed proof state, inspects Schedule Service execution, or uses the manual seller / buyer resale flow. Final demo proof links for bookings `193` and `194` also render product-native recorded recovery receipts when live Redis state has moved on. | `GET /api/wallet-budget/config`, `POST /api/wallet-budget/allowance-request`, `POST /api/recovery/preview`, `POST /api/recovery/confirm`, `POST /api/automation/inspect`, `POST /api/resale-list`, `POST /api/resale-buy` |
...
| `/week5-proof` | `app/week5-proof/page.tsx` | Hidden, noindex reviewer proof for Hedera AI Bounty Week 5 | — (Server Component) | Inspect Agent Kit policy hooks, HBAR/USDC x402 settlement tx ids, wallet-funded USDC allowance, and JSON proof API. Not linked from product nav. | — |
...
| `RecoveryConciergePanel` | `components/concierge/RecoveryConciergePanel.tsx` | Plain-English recovery guide, policy preview, explicit approval, and receipt surface for resale listing or release/refund Concierge flows | `/resale/[serial]` |
| `WalletBudgetConnector` | `components/wallet/WalletBudgetConnector.tsx` | Optional Hedera WalletConnect/Reown path for approving a bounded HTS/USDC allowance budget from a connected wallet | `/resale/[serial]` |
...
| `/api/wallet-budget/config` | GET | Browser-safe config for optional Hedera WalletConnect allowance approval: USDC token id, spender account id, and bounded allowance amount | Yes — Resale recovery panel |
| `/api/wallet-budget/allowance-request` | POST | Builds the unsigned HTS/USDC allowance transaction bytes for the connected wallet to sign and execute through Hedera WalletConnect | Yes — Wallet budget panel |
...
| `/api/agent/week5-proof` | GET | Machine-readable Hedera Policy Agent proof bundle for reviewers: Agent Kit runtime policies, tool discovery, x402 HBAR/USDC settlement refs, USDC allowance ref, protocols, and manifest tools | **No** — hidden reviewer/verifier integration |
| `/api/nft-studio/proof` | GET | Hedera NFT Studio support proof for booking-right NFT metadata validation and token-control risk review | **No** — hidden reviewer/verifier integration |
| `/api/nft-studio/booking-rights/[serial]/image` | GET | Generated SVG image used by booking-right HIP-412-style metadata and NFT Studio metadata validation | **No** — NFT metadata asset endpoint |
| `/api/x402/recovery-policy` | GET/POST | Hedera x402 exact payment-required endpoint for paid recovery-policy quotes; advertises HBAR and HTS/USDC requirements and can verify/settle through a configured facilitator when `HEDERA_X402_SETTLEMENT_ENABLED=true` | **No** — external x402 client / verifier integration |
...
| **F2** Resale + royalty | Recover/list and buy | `/my-bookings` → `/resale/[serial]?mode=recovery`, plus manual `/resale/[serial]`; optional Telegram transport through `/api/telegram/webhook` | Concierge preview + approval creates a listing; proof receipt persists in demo state; approved recovery also creates a Schedule Service payment proof and Hedera Agent Kit proof trace; Telegram accepts plain-English intents like "I can't make booking 123", "Sell booking 123", and "Refund booking 123" before routing to the same allowlisted/mutation-gated actions; final recorded proof for booking `193` remains visible as a product receipt if live state is later reset; manual seller and buyer dialogs remain available; royalty copy on page + `lib/domain/fees.ts` |
