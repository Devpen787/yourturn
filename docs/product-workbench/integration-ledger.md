...
Concrete signed-route harness in `scripts/world-recovery-live-proof.mjs`:
- `POST /api/agent/confirm` with an official AgentKit-signed request;
- public result expects `worldTrust.signal === "human-backed-agent"`;
- exact registered delegated-agent check;
- privacy-minimized output with `humanIdExposed: false`.

Current evidence is **LIVE/AGENTBOOK** for production registration/resolution and **CI/READY, NOT LIVE** for the bounded signed `create_listing` route harness. The registered-agent signed recovery mutation has not yet been executed against the required initialized non-production YourTurn target. The harness keeps the registered-agent private key local and must not be promoted to LIVE until that execution is completed and the exact result is independently reviewed. World ID Sandbox evidence is also still required separately.

The current harness obtains a legacy approval grant before confirm, so the Integrator must bind the World trust result to the Recovery-Mandate recovery path without routing Ledger authority through the legacy reusable approval-grant model. World proves the requester; it does not grant booking ownership/provider entitlement or define holder authority.
...
| YT-06 exact delegated agent verification | UI says `Exact delegated agent verified` and `Human-backed` without identifying the human. | Agent trust presentation | World | **FIXTURE** UI; AgentBook resolution is **LIVE**; signed `create_listing` route harness is **CI/READY, NOT LIVE** | AgentKit-signed request + AgentBook/exact-agent resolution integrated into the actual Recovery-Mandate execution path | Preserve `human-backed-agent` signal; exact delegated-agent match; privacy-safe error/output; execute and independently review the registered-agent signed route; complete separate Sandbox proof; bind proof to the recovery resource, not merely a separate listing demo | Verification mismatch/failure blocks execution and remains privacy-safe; never log/display raw World human identifier. |
