...
- `2026-06-19`: Hedera AI Bounty Week 5 candidate path = submit YourTurn Concierge as a policy-constrained HBAR recovery agent; runtime now attaches official Hedera Agent Kit `MaxRecipientsPolicy(1)` and `RejectToolPolicy`, while the existing domain policy layer remains responsible for holder, provider-policy, approval, refund, schedule, and budget checks.
- `2026-06-19`: Week 5 booster path = implement WalletConnect/Reown only as a user-signed bounded HTS/USDC allowance request, add Agent Lab and NFT Studio as reviewer proof artifacts, and keep raw autonomous private-key signing out of scope.
- `2026-06-19`: Concierge UX should lead with plain customer intents ("someone else can take my spot", "give it back", "refund booking 123") and keep Hedera Agent Kit, HCS, x402, and policy details in proof/receipt layers rather than as the first interaction language.
